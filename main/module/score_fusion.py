...
def plot_confusion_matrix(y_test_final, y_predict):
    ...
    fig, ax = plt.subplots()
    im = ax.imshow(cm, cmap=plt.cm.Blues, interpolation='nearest', extent=(-0.5,
                   len(np.unique(y_test_final))-0.5, len(np.unique(y_test_final))-0.5, -0.5))
...
def score_fusion(iris_scores, peri_score, hd_thresh=0.471, jd_thresh=0.651, tdi_thresh=0.642, alpha_0=0.5, alpha_inc=10):
    try:
        # Calculate the deviation of the iris scores from the thresholds
        iris_score_dev = [[max(0, hd - hd_thresh), max(0, jd - jd_thresh),
                           max(0, tdi - tdi_thresh)] for [hd, jd, tdi] in iris_scores]

        # Calculate the average deviation of the iris scores from the thresholds
        num_pass = sum([hd <= hd_thresh and jd <= jd_thresh and tdi <=
                        tdi_thresh for [hd, jd, tdi] in iris_scores])
        if num_pass == 0:
            avg_dev = 0
        else:
            avg_dev = sum([sum(dev_score) for dev_score, [hd, jd, tdi] in zip(
                iris_score_dev, iris_scores) if hd >= hd_thresh and jd >= jd_thresh and tdi >= tdi_thresh]) / num_pass
        ...
        iris_scores_weighted = [1 if hd <= hd_thresh and jd <= jd_thresh and tdi <=
                                tdi_thresh else 0 for [hd, jd, tdi] in iris_scores]
        combined_score = alpha * \
            max(iris_scores_weighted) + (1 - alpha) * peri_score
...
def accuracy_score_multi_thread(labels, model, iris_norm_L, iris_norm_R, X_test, num_threads=2 * os.cpu_count()):
    predict = []
    ground_truth = []
    total_test_img = 10
