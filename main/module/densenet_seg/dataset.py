...
import copy
from module.densenet_seg.utils import one_hot2dist
...
class Starburst_augment(object):
    ...
    def __call__(self, img):
        ...
        if mode == 0:
            starburst = np.pad(starburst, pad_width=(
                (0, 0), (x, 0)), mode='constant')
            starburst = starburst[:, :-x]
        if mode == 1:
            starburst = np.pad(starburst, pad_width=(
                (0, 0), (0, x)), mode='constant')
            starburst = starburst[:, x:]
...
#        print (mode,factor_h,factor_v)
        if mode == 0:
            aug_base = np.pad(base, pad_width=(
                (factor_v, 0), (0, 0)), mode='constant')
            aug_mask = np.pad(mask, pad_width=(
                (factor_v, 0), (0, 0)), mode='constant')
            aug_base = aug_base[:-factor_v, :]
            aug_mask = aug_mask[:-factor_v, :]
        if mode == 1:
            aug_base = np.pad(base, pad_width=(
                (0, factor_v), (0, 0)), mode='constant')
            aug_mask = np.pad(mask, pad_width=(
                (0, factor_v), (0, 0)), mode='constant')
            aug_base = aug_base[factor_v:, :]
            aug_mask = aug_mask[factor_v:, :]
...
class Line_augment(object):
    def __call__(self, base):
        ...
        for i in np.arange(0, num_lines):
            ...
            aug_base = cv2.line(aug_base, (x1, y1),
                                (x2, y2), (255, 255, 255), 4)
...
class IrisDataset(Dataset):
    ...
    def __getitem__(self, idx):
        imagepath = osp.join(self.filepath, 'images',
                             self.list_files[idx]+'.png')
        ...
        if self.split != 'test':
            labelpath = osp.join(self.filepath, 'labels',
                                 self.list_files[idx]+'.npy')
        ...
        if self.split != 'test':
            ...
            spatialWeights = cv2.dilate(
                spatialWeights, (3, 3), iterations=1)*20
...
if __name__ == "__main__":
    ...
    ds = IrisDataset('Semantic_Segmentation_Dataset',
