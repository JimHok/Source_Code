...
class GeneralizedDiceLoss(nn.Module):
    ...
    def forward(self, ip, target):
        ...
        Label = (np.arange(4) == target.cpu().numpy()
        ...
        ip = torch.flatten(ip, start_dim=2, end_dim=-
                           1).cuda().to(torch.float32)
        target = torch.flatten(target, start_dim=2,
                               end_dim=-1).cuda().to(torch.float32)

        numerator = ip*target
        denominator = ip + target

        class_weights = 1./(torch.sum(target, dim=2) **
                            2).clamp(min=self.epsilon)
