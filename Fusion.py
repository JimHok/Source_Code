...
def accuracy_score_multi_thread(ref_num, test_num, same_num=2):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for t, true, f, false, p, g in tqdm(executor.map(process_images, image_combinations), 
                                            ...
                                            miniters=int(ref_num*test_num*(same_num-1)/6000),
                                            maxinterval=20000):
            tar += t
            trr += true
            far += f
...
confu, predict, ground_truth = accuracy_score_iris(ref_img_num, test_img_num, same_num=4, thresh=0.5)
plot_confu(ground_truth, predict, f'Confu_matrix_{ref_img_num}_{test_img_num}_4_iris_5.png')
